...
import { Star, Clock, MapPin, Users, Calendar, Plus, Minus, CheckCircle2, Sparkles, Badge as BadgeIcon, Info, Filter, X, TrendingUp, AlertCircle, CalendarDays } from "lucide-react";
...
const ProductDetail = ({ onNext, onBack, tourData }: ProductDetailProps) => {
  ...
  const filteredTours = mockTours
    .filter((tour) => {
      ...
      if (tourData?.searchCategory && tourData.searchCategory !== "all") {
        const mappedCategories = categoryNameToTourCategories[tourData.searchCategory] || [];
        if (mappedCategories.length > 0) {
          categoryMatch = categoryMatch && mappedCategories.some((cat) => 
            tour.category.toLowerCase().includes(cat.toLowerCase())
          );
        }
      }
  ...
  const getBookingDetails = (tour: any) => {
    ...
    const totalPrice = (popupAdultCount * tour.price) + (popupChildCount * childPrice);
    return { adultCount: popupAdultCount, childCount: popupChildCount, timeSlot: popupTimeSlot, price: totalPrice };
  };
  ...
  const handleOpenPopup = (tour: any) => {
    ...
    if (isAdultOnly) { adultCount = adultCount || 1; childCount = 0; }
    else if (isChildOnly) { adultCount = 0; childCount = childCount || 1; }
  ...
  const handleConfirmBooking = () => {
    ...
    if (tour) {
      ...
      onNext({ 
        ...
        date: popupDate ? format(popupDate, 'yyyy-MM-dd') : (tourData?.date || ''),
        categories: tourData?.categories || [],
        tour,
        bookingDetails: { ...bookingDetails, date: popupDate ? format(popupDate, 'yyyy-MM-dd') : (tourData?.date || ''), timeSlot: popupTimeSlot }
      });
        ...
        <div className="flex flex-col gap-1.5 sm:flex-row sm:items-center sm:justify-between w-full min-w-0">
          <div className="space-y-0.5 min-w-0 flex-1">
            ...
            <p className="text-xs sm:text-sm text-muted-foreground">
              Found <span className="font-semibold text-primary">{filteredTours.length}</span> experiences in {tourData?.destination || "your destination"}
            </p>
          </div>
